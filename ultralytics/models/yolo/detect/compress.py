...
# 没有购买yolov11项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Detect_LSCD, Detect_RSCD, Detect_TADDH,Detect_LSDECD
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, SimFusion_4in, AdvPoolFusion, \
    PyramidPoolAgg, RepVGGBlock, RepConvN, Star_Block, Faster_Block_CGLU, MutilScaleEdgeInformationEnhance, CSPOmniKernel, SPDConv,FocusFeature
...
from ultralytics.nn.backbone.fasternet import MLPBlock,FasterNet
...
def get_pruner(opt, model, example_inputs):
    ...
    # for yolo11s-fasternet-FDPN.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect):
    #         ignored_layers.append(m.cv2[0][2])
    #         ignored_layers.append(m.cv2[1][2])
    ...
    #     if isinstance(m, AAttn):
    #         ignored_layers.append(m)
    #     if isinstance(m, FocusFeature):
    #         ignored_layers.append(m)
    # ignored_layers.append(model.model[2].cv2)
    # ignored_layers.append(model.model[9].cv2)

    # for yolo11s-FDPN.yaml
    # for k, m in model.named_modules():
    #     if isinstance(m, Detect):
    #         ignored_layers.append(m.cv2[0][2])
    #         ignored_layers.append(m.cv2[1][2])
    ...
    #yolo11-fasternet-FDPN-LSDECD.yaml
    # for k, m in model.named_modules():
